...
# Example usage
origin = ((X_MAX + X_MIN) / 2, (Y_MAX + Y_MIN) / 2)  # Correct center calculation
side_length = 10.0
angle_degrees = 180  # Rotate counterclockwise

acceleration = 0.1
for i in range(0, 720, 10):
    path = draw_equilateral_triangle(origin, side_length, i)
    plotter.layers[black_pen_layer].add_path(path)
    side_length += acceleration
    acceleration += 0.1
    # origin = (origin[0] + 5, origin[1] + 5)
